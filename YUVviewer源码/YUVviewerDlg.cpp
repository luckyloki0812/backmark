...
UINT PlayVideo( LPVOID pParam )
{
	...
	while(g_nCurrentFrame >= g_nStartFrame && g_nCurrentFrame <= g_nEndFrame && !bEof)
	{
		...
		for(i=0; i<pWin->m_iCount; i++)
		{
			pWin->m_pFile[i]->Seek(g_nCurrentFrame*picsize, SEEK_SET);//Seek 函数 返回一个 Long，在 Open 语句打开的文件中指定当前的读/写位置。
...
void CYUVviewerDlg::OnPauseplay() 
{
	...
	{
		WaitForSingleObject( hPlayTemp,INFINITE);//WaitForSingleObject函数用来检测hHandle事件的信号状态，在某一线程中调用该函数时，线程暂时挂起，如果在挂起的dwMilliseconds毫秒内，线程所等待的对象变为有信号状态，则该函数立即返回；如果超时时间已经到达dwMilliseconds毫秒，但hHandle所指向的对象还没有变成有信号状态，函数照样返回。
...
//	CYUVviewerDlg* pWin = (CYUVviewerDlg*)pParam;
	...
	// add by wyn
		...
		//				else
		//				{
		//					*pTemp = (BYTE)((*pOldData)*0.55+g_arrMBData[nMBType][0]*0.45);
		//					pOldData++;
		//					pTemp++;
		//					
		//					*pTemp = (BYTE)((*pOldData)*0.55+g_arrMBData[nMBType][1]*0.45);
		//					pOldData++;
		//					pTemp++;
		//					
		//					*pTemp = (BYTE)((*pOldData)*0.55+g_arrMBData[nMBType][2]*0.45);
		//					pOldData++;
		//					pTemp++;
		//				}
		//			}
...
void CYUVviewerDlg::OnPrevious5() //向前进5帧(相当于倒带)
{
	...
	for(i=0; i<m_iCount; i++)
	{
		m_pFile[i]->Seek(g_nCurrentFrame*picsize, SEEK_SET);//定位到文件的某个位置(应该是帧的开始)//#define SEEK_SET    0
																//
		m_pWnd[i]->nPicShowOrder = g_nCurrentFrame;
	}
	...
	if(g_nCurrentFrame < g_nEndFrame) // && !bEof)	//当前帧数<最后的帧数，即未到尾
	{
		...
		for(i=0; i<m_iCount; i++)
		{
			...
			if(0)//这儿肯定会执行啊，加这个if貌似多余//bColorImage) 
			{
